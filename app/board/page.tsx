...
import { EpisodePhase, PHASE_BORDER_STYLE, STAGE_TO_PHASE } from "../episodes/phases";
        ...
        {STAGE_ORDER.map((stage) => {
          ...
          return (
            <div key={stage} className="w-64 shrink-0 rounded-lg bg-slate-50 shadow-sm">
              <div
                className={`flex items-center justify-between rounded-t-md border-t-4 px-3 py-2 ${PHASE_BORDER_STYLE[phase]} ${COLUMN_BG_STYLE[phase]}`}
              >
                  ...
                  return (
                    <li key={episode.id}>
                      <Link
                        ...
                        className={`block rounded-md border-l-4 bg-white p-2.5 text-sm shadow-sm transition-shadow hover:shadow-md ${PHASE_BORDER_STYLE[phase]}`}
                      >
